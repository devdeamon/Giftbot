      ...
      ? [
          {
            ...
            description: "Mining rewards earned",
          },
        ]
      : []),
    {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center space-y-2 border border-green-400/30 p-4 bg-black/50">
        ...
        <p className="text-green-600 text-xs uppercase tracking-widest">FINANCIAL PROTOCOL ACTIVE</p>
      </div>
      ...
      <Card className="bg-black border-green-400/30 border">
        ...
        <CardContent className="space-y-4 p-4">
          <div className="text-center space-y-2">
            ...
            <div className="text-xs text-green-600 uppercase tracking-wider">GIFT_TOKENS</div>
            <div className="text-xs text-green-700">{"$".repeat(Math.min(20, Math.floor(giftBalance * 1000)))}</div>
          </div>
          <div className="flex justify-center">
            <Badge className="bg-green-400/20 text-green-400 border-green-400/30">
              {giftBalance > 0 ? "WALLET_ACTIVE" : "WALLET_EMPTY"}
            </Badge>
          </div>
          <div className="text-xs text-green-700 text-center font-mono">
            SHARDS_CONVERTED: {shardsFound.toString().padStart(6, "0")}
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-3 p-4">
          {transactions.length > 0 ? (
            ...
            <div className="text-center text-green-700 space-y-2">
              ...
              <div className="text-xs uppercase tracking-wider">NO_TRANSACTIONS_FOUND</div>
              <div className="text-xs">
                {">"} START_MINING_TO_EARN_TOKENS {"<"}
              </div>
            </div>
          )}
        </CardContent>
      ...
      {/* Send Modal */}
      <Modal isOpen={isSendModalOpen} onClose={() => setIsSendModalOpen(false)} title="SEND_TOKENS">
        <div className="space-y-4 text-green-400">
          <div className="space-y-2">
            <label className="text-xs text-green-600 uppercase">RECIPIENT_ADDRESS:</label>
            <input
              type="text"
              value={sendAddress}
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full bg-black border border-green-400/30 text-green-400 p-2 text-xs font-mono"
            />
            <div className="text-xs text-green-700">AVAILABLE: {giftBalance.toFixed(6)} GIFT</div>
          </div>
          <div className="space-y-2">
            <Button
      ...
      {/* Receive Modal */}
      <Modal isOpen={isReceiveModalOpen} onClose={() => setIsReceiveModalOpen(false)} title="RECEIVE_TOKENS">
        <div className="space-y-4 text-green-400">
          <div className="text-center space-y-4">
            ...
            <div className="space-y-2">
              <div className="text-xs text-green-600 uppercase">YOUR_WALLET_ADDRESS:</div>
          ...
          <div className="text-xs text-green-700 text-center font-mono border border-green-400/20 p-2">
            SHARE_THIS_ADDRESS_TO_RECEIVE_TOKENS
          </div>
        </div>
      </Modal>
    </div>
